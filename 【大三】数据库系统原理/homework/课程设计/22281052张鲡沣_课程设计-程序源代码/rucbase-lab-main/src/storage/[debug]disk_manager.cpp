...
void DiskManager::write_page(int fd, page_id_t page_no, const char *offset, int num_bytes) {
    off_t expected_offset = static_cast<off_t>(page_no) * PAGE_SIZE;
    if (lseek(fd, expected_offset, SEEK_SET) != expected_offset) {
        throw UnixError();
    }
...
void DiskManager::read_page(int fd, page_id_t page_no, char *offset, int num_bytes) {
    ...
    if (expected_offset >= st.st_size) {
        // 如果要读的位置超出了文件大小，这本身就是一个错误信号
        // 但为了让上层知道，我们还是抛出原来的错误
        throw InternalError("DiskManager::read_page Error: Attempt to read beyond file size.");
    }
    ...
    if (bytes_read < 0) { // read返回负数是真正的错误
        throw UnixError();
    }
    if (bytes_read != num_bytes) {
        // 如果读到的字节数不足，但没有错误，说明读到了文件末尾
        // 这在页式管理中也应该是一个错误
        throw InternalError("DiskManager::read_page Error: Incomplete page read.");
    }
}
