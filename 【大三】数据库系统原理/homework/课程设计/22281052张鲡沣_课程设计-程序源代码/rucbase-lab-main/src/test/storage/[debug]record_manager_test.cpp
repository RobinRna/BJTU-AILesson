...
void check_equal(const RmFileHandle *file_handle,
    ...
    std::cout << "[DEBUG] check_equal called. num_pages = " << file_handle->file_hdr_.num_pages << std::endl;                
...
TEST(RecordManagerTest, SimpleTest) {
    ...
    for (int round = 0; round < 1000; round++) {
        ...
        if (round % 50 == 0) {
            std::cout << "[DEBUG] Closing file. num_pages = " << file_handle->file_hdr_.num_pages << ", first_free = " << file_handle->file_hdr_.first_free_page_no << std::endl;
            rm_manager->close_file(file_handle.get());
            file_handle = rm_manager->open_file(filename);
        }
