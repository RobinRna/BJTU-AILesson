...
// Helper function to persist the file header.
static void persist_file_hdr(RmFileHandle* file_handle) {
    Page* hdr_page = file_handle->buffer_pool_manager_->fetch_page({file_handle->GetFd(), RM_FILE_HDR_PAGE});
    assert(hdr_page != nullptr);
    memcpy(hdr_page->get_data(), &(file_handle->file_hdr_), sizeof(RmFileHdr));
    file_handle->buffer_pool_manager_->unpin_page({file_handle->GetFd(), RM_FILE_HDR_PAGE}, true);
}
...
Rid RmFileHandle::insert_record(char* buf, Context* context) {
    ...
    // Use the newly discovered Bitmap::first_bit function
...
void RmFileHandle::delete_record(const Rid& rid, Context* context) {
    ...
    // Correct logic: check if it was full BEFORE decrementing
...
RmPageHandle RmFileHandle::fetch_page_handle(int page_no) const {
    ...
    // Corrected: Use lowercase function name
