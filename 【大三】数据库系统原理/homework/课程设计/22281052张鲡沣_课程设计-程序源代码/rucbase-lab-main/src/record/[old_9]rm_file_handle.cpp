...
RmFileHandle::~RmFileHandle() {
    // This is the final and most important persistence point.
    // When the file handle is closed (and this object is destroyed),
    // we must write the final state of the file header back to disk.
    Page* hdr_page = buffer_pool_manager_->fetch_page({fd_, RM_FILE_HDR_PAGE});
    if (hdr_page != nullptr) {
        memcpy(hdr_page->get_data(), &file_hdr_, sizeof(file_hdr_));
        buffer_pool_manager_->unpin_page({fd_, RM_FILE_HDR_PAGE}, true);
        // We should also flush it to be absolutely safe, though unpinning dirty should suffice.
        buffer_pool_manager_->flush_page({fd_, RM_FILE_HDR_PAGE}); 
    }
}