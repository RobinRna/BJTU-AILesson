...
#include "errors.h" // 包含了 PageFullError, SlotOccupiedError, RecordNotFoundError 等
...
std::unique_ptr<RmRecord> RmFileHandle::get_record(const Rid& rid, Context* context) const {
    ...
    // Correct way to create RmRecord with rid
    auto record = std::make_unique<RmRecord>(rid, file_hdr_.record_size);
...
Rid RmFileHandle::insert_record(char* buf, Context* context) {
    ...
    int slot_no = Bitmap::find_first_free(page_handle.bitmap, file_hdr_.num_records_per_page);
...
void RmFileHandle::delete_record(const Rid& rid, Context* context) {
    ...
    Bitmap::reset(page_handle.bitmap, rid.slot_no); // Corrected: Use reset instead of free
    page_handle.page_hdr->num_records--;
...
RmPageHandle RmFileHandle::fetch_page_handle(int page_no) const {
    ...
    if (page_no < 0 || page_no >= file_hdr_.num_pages) {
        // Corrected: Pass a string (can be empty) as the first argument
        throw PageNotExistError("", page_no);
    }
    Page* page = buffer_pool_manager_->fetch_page({fd_, page_no});
    if (page == nullptr) {
        // Corrected: Pass a string as the first argument
        throw PageNotExistError("", page_no);
    }
...
RmPageHandle RmFileHandle::create_new_page_handle() {
    ...
    if (page == nullptr) {
        throw PageFullError(); // This error is defined in errors.h
    }
